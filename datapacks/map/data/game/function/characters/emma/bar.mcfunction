...
execute as @s[scores={Mana=0..}] run item replace entity @s hotbar.5 with netherite_ingot[custom_name='[{"text":"Pure Souls","color":"gold","italic":false,"bold":true},{"text":" : Souls Heals 2 Extra (passive effect)","color":"white","italic":false,"bold":false}]',lore=['{"text":"+6 Health (+4 for every player over 2)","color":"white","italic":false}']]
