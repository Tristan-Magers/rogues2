...
execute as @s[scores={timer=1006}] at @s run summon area_effect_cloud ~ ~.6 ~ {Particle:{type:"crit"},ReapplicationDelay:0,RadiusPerTick:-.05f,Radius:3f,Duration:60,potion_contents:{potion:"minecraft:harming"},Tags:["thorns"]}
execute as @s[scores={timer=1006}] at @s run summon area_effect_cloud ~ ~.6 ~ {Particle:{type:"crit"},ReapplicationDelay:0,RadiusPerTick:-.05f,Radius:3f,Duration:60,potion_contents:{custom_effects:[{id:"minecraft:luck",amplifier:1,duration:30,show_particles:0b,show_icon:0b}]}}
